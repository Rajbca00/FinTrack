...
async function fetchTransactions(filters: { from?: Date; to?: Date; accountId?: string; groupId?: string | string[]; bucketId?: string }) {
...
export async function getCategoryBreakdown(filters: { from?: Date; to?: Date; accountId?: string; groupId?: string | string[]; bucketId?: string; type?: "INCOME" | "EXPENSE" }) {
  const txns = await fetchTransactions(filters);
