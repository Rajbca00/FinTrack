...
const CATEGORIES: { name: string; type: CategoryType; color: string; isSystem?: boolean }[] = [
...
const RULES: { pattern: string; matchType: MatchType; category: string; amountSign?: string }[] = [
...
async function main() {
  ...
  for (const r of RULES) {
    ...
    const existing = await prisma.categoryRule.findFirst({ where: { pattern: r.pattern, categoryId } });
    ...
    await prisma.categoryRule.create({
      data: {
        ...
        categoryId,
        amountSign: r.amountSign ?? "ANY",
        priority: 0,
      },
    });
  }

  console.log("Seed complete:", categoryIdByName.size, "categories,", RULES.length, "rules");
}
